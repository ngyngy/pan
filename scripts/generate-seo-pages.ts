...
${escapeHtml(res.recommendation)}
      ...
      <!-- 网盘下载与转存通道 (推特/X 专属防封免拦截) -->
      <h2>🔗 网盘直接转存通道 (免封直达)</h2>
      <div class="download-box" style="position:relative;">
        <div style="display:inline-flex; align-items:center; gap:6px; background:#eff6ff; color:#1d4ed8; padding:4px 10px; border-radius:6px; font-size:12px; font-weight:bold; margin-bottom:12px;">
          <span>🐦 Twitter / X 专属防封安全通道 · 100% 顺畅打开</span>
        </div>

        <p style="margin:0 0 10px 0; font-weight:600;">👉 ${escapeHtml(res.driveName)}分享地址：</p>
        <p style="margin:0 0 12px 0; word-break:break-all;">
          <a href="${escapeHtml(res.driveUrl)}" target="_blank" rel="noopener noreferrer" style="color:#059669; font-weight:bold; text-decoration:underline;">
            ${escapeHtml(res.driveUrl)}
          </a>
        </p>

        ${res.extractCode ? `
        <div style="background:#fff; border:1px solid #10b981; border-radius:8px; padding:10px 14px; margin-bottom:14px; display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:8px;">
          <div>
            <span style="font-size:13px; color:#4b5563;">提取口令 / 提取码：</span>
            <strong id="extractCodeText" style="font-size:16px; font-family:monospace; color:#065f46; letter-spacing:1px; background:#ecfdf5; padding:2px 8px; border-radius:4px;">${escapeHtml(res.extractCode)}</strong>
          </div>
          <button type="button" onclick="copyText('${escapeHtml(res.extractCode)}', '提取口令已成功复制！')" style="background:#10b981; color:#fff; border:none; padding:6px 14px; border-radius:6px; font-size:13px; font-weight:bold; cursor:pointer;">
            一键复制口令
          </button>
        </div>
        ` : ''}
        ...
        <div style="display:flex; flex-wrap:wrap; gap:10px; margin-top:14px;">
          <a href="${escapeHtml(res.driveUrl)}" target="_blank" rel="noopener noreferrer" class="btn" style="flex:1; min-width:200px; text-align:center;">
            🚀 立即打开${escapeHtml(res.driveName)}转存资源
          </a>
          <button type="button" onclick="copyText('${escapeHtml(res.driveUrl)}', '网盘链接已成功复制！')" style="background:#f3f4f6; color:#374151; border:1px solid #d1d5db; padding:12px 18px; border-radius:8px; font-weight:600; font-size:14px; cursor:pointer;">
            📋 复制分享链接
          </button>
          <a href="${spaDirectUrl}" style="background:#fff; color:#2563eb; border:1px solid #93c5fd; padding:12px 18px; border-radius:8px; font-weight:600; font-size:14px; text-decoration:none; display:inline-block;">
            🖥️ 进入全功能交互主站
          </a>
        </div>
      </div>
