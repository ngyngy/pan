...
<body>
  <div class="container">
    ...
    <article class="card">
      ...
      <!-- 【核心直达】夸克网盘分享链接与极速转存通道 (位于标题下方、AI智能搜索摘要上方) -->
      <section class="top-direct-card" aria-label="夸克网盘直接转存与下载通道" style="margin-bottom:20px;">
        <div style="display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:10px; margin-bottom:14px; border-bottom:1px dashed rgba(16,185,129,0.35); padding-bottom:12px;">
          <div style="display:inline-flex; align-items:center; gap:8px;">
            <span style="font-size:22px;">⚡</span>
            <h2 style="margin:0; border:none; padding:0; font-size:18px; font-weight:800; color:#059669;">
              ${escapeHtml(res.driveName)}直接转存通道 (免封直达 · 点击即存)
            </h2>
          </div>
          <div style="display:inline-flex; align-items:center; gap:6px; background:#eff6ff; color:#1d4ed8; padding:5px 12px; border-radius:9999px; font-size:12px; font-weight:bold; border:1px solid #bfdbfe;">
            <span>🐦 Twitter / 𝕏 官方专属安全通道 · 100% 顺畅免拦截</span>
          </div>
        </div>

        <!-- 夸克网盘分享地址展示区 -->
        <div class="top-link-box">
          <p style="margin:0 0 6px 0; font-weight:700; font-size:15px; color:#059669;">
            👉 ${escapeHtml(res.driveName)}分享地址：
          </p>
          <p style="margin:0; word-break:break-all;">
            <a href="${escapeHtml(res.driveUrl)}" target="_blank" rel="noopener noreferrer" style="color:#059669; font-weight:bold; font-size:17px; text-decoration:underline; line-height:1.5;">
              ${escapeHtml(res.driveUrl)}
            </a>
          </p>
        </div>

        <!-- ⚠️ 推特/𝕏 内置浏览器专属安全提示与复制指引 (防止内置 Webview 拦截遮蔽) -->
        <div class="twitter-app-tip" id="twitterAppTip">
          <div style="display:flex; align-items:flex-start; gap:10px;">
            <span style="font-size:20px; line-height:1.2;">⚠️</span>
            <div style="flex:1; font-size:13px; line-height:1.6;">
              <strong style="font-size:14px; display:block; margin-bottom:4px; color:#b45309;">
                推特 / 𝕏 手机端用户特别提醒（防止外链被屏蔽）：
              </strong>
              <span>如果您当前是在 <strong>推特 / 𝕏 APP 内置浏览器</strong> 中打开本页，直接点击网盘链接极易被推特安全策略拦截或屏蔽。</span>
              <div style="margin-top:6px; padding:8px 12px; background:rgba(245,158,11,0.12); border-radius:8px; border:1px dashed #f59e0b; font-weight:600;">
                💡 <strong>正确操作步骤：</strong><br>
                ① 点击下方【📋 复制分享链接】按钮；<br>
                ② 切换到手机系统浏览器（Safari / Chrome）粘贴打开，或直接打开【夸克APP】即可秒级转存！
              </div>
            </div>
          </div>
        </div>
        ...
        ${res.extractCode ? `
        <!-- 提取码 / 口令 -->
        <div style="background:#fff; border:1px solid #10b981; border-radius:10px; padding:10px 14px; margin-bottom:14px; display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:8px;">
          <div>
            <span style="font-size:14px; color:#374151; font-weight:600;">提取口令 / 提取码：</span>
            <strong id="extractCodeText" style="font-size:18px; font-family:monospace; color:#065f46; letter-spacing:1px; background:#ecfdf5; padding:3px 10px; border-radius:6px; border:1px solid #a7f3d0;">${escapeHtml(res.extractCode)}</strong>
          </div>
          <button type="button" onclick="copyText('${escapeHtml(res.extractCode)}', '提取口令已成功复制！')" style="background:#10b981; color:#fff; border:none; padding:8px 16px; border-radius:8px; font-size:13px; font-weight:bold; cursor:pointer;">
            一键复制口令
          </button>
        </div>
        ` : ''}
        ...
        <div style="display:flex; flex-wrap:wrap; gap:10px; margin-top:10px;">
          <a href="${escapeHtml(res.driveUrl)}" target="_blank" rel="noopener noreferrer" class="btn" style="flex:1; min-width:220px; text-align:center; font-size:16px; padding:13px 22px; justify-content:center; box-shadow:0 4px 12px rgba(16,185,129,0.3);">
            🚀 立即打开${escapeHtml(res.driveName)}转存资源
          </a>
          <button type="button" onclick="copyText('${escapeHtml(res.driveUrl)}', '网盘链接已成功复制！')" style="background:#f3f4f6; color:#1f2937; border:1px solid #d1d5db; padding:12px 18px; border-radius:8px; font-weight:700; font-size:14px; cursor:pointer;">
            📋 复制分享链接
          </button>
          <a href="${spaDirectUrl}" style="background:#fff; color:#2563eb; border:1px solid #93c5fd; padding:12px 18px; border-radius:8px; font-weight:700; font-size:14px; text-decoration:none; display:inline-flex; align-items:center; gap:6px;">
            🖥️ 进入主站全功能界面
          </a>
        </div>
      </section>
...
${escapeHtml(res.recommendation)}
  ...
  <script>
    ...
    // 智能识别 Twitter / 𝕏 内置 WebView 浏览器环境并重点高亮指引
    (function checkTwitterEnvironment() {
      var ua = navigator.userAgent || '';
      var isTwitter = /Twitter/i.test(ua);
      var tipEl = document.getElementById('twitterAppTip');
      if (tipEl) {
        if (isTwitter) {
          tipEl.style.border = '2px solid #ef4444';
          tipEl.style.background = '#fef2f2';
          tipEl.style.color = '#991b1b';
          tipEl.innerHTML = '<div style="display:flex;align-items:flex-start;gap:10px;"><span style="font-size:24px;">🚨</span><div><strong style="font-size:15px;display:block;margin-bottom:4px;color:#dc2626;">已检测到您正在使用 Twitter / 𝕏 内置浏览器！</strong><p style="margin:0 0 6px 0;font-size:13px;line-height:1.5;">推特内置浏览器会拦截外部网盘链接。请<strong>切勿直接点击链接</strong>，请点击下方<strong>【📋 复制分享链接】</strong>，然后切换到手机 Safari / Chrome 浏览器中打开，或直接打开<strong>【夸克APP】</strong>即可直接转存！</p></div></div>';
        }
      }
    })();
  </script>
