...
lines.forEach((line, idx) => {
  ...
  if (match) {
    ...
    const code = codeMatch ? codeMatch[1].toLowerCase() : `b${idx + 1}`;
    const id = `bilibili-paid-${code}`;
    
    // 提炼关键标签
    const tags = ['B站付费课', '名师精品课', '夸克网盘'];
    const cleanTitle = title.replace(/（完结）|\(完结\)|\(更新中\)|\【完结\】|\[B站\]|-/g, ' ').replace(/\s+/g, ' ').trim();
    
    if (title.includes('理财') || title.includes('金融') || title.includes('经济') || title.includes('商业') || title.includes('汇率')) {
      tags.push('商业金融', '财经通识');
    }
    if (title.includes('Python') || title.includes('Java') || title.includes('Web前端') || title.includes('编程') || title.includes('Linux') || title.includes('算法') || title.includes('深度学习') || title.includes('AI')) {
      tags.push('IT编程', '计算机技术');
    }
    if (title.includes('摄影') || title.includes('剪辑') || title.includes('PR') || title.includes('C4D') || title.includes('PS') || title.includes('AE') || title.includes('特效') || title.includes('后期') || title.includes('设计')) {
      tags.push('影音剪辑', '视觉设计');
    }
    if (title.includes('哲学') || title.includes('史') || title.includes('历史') || title.includes('鲁迅') || title.includes('红楼梦') || title.includes('唐诗') || title.includes('文学') || title.includes('国学')) {
      tags.push('人文社科', '名家通识');
    }
    if (title.includes('英语') || title.includes('音标') || title.includes('口语') || title.includes('词汇')) {
      tags.push('英语进阶', '语言学习');
    }
    if (title.includes('减脂') || title.includes('塑身') || title.includes('篮球') || title.includes('增肌') || title.includes('体态') || title.includes('唱歌')) {
      tags.push('个人成长', '生活才艺');
    }
    ...
    const description = `【B站付费精品名师课】《${cleanTitle}》完整版视频与配套讲义课件！夸克网盘免密极速转存，涵盖系统讲解、案例实战与高阶进阶技巧，支持手机/PC随时随地学习。`;
...
console.log(`Successfully parsed ${parsed.length} items from ${lines.length} lines.`);

fs.writeFileSync('scripts/parsed_bilibili_courses.json', JSON.stringify(parsed, null, 2), 'utf8');
