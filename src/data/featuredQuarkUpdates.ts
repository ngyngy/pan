import { ResourceItem } from '../types';

/**
 * 最新精选夸克网盘更新资源
 * 包含官方直发推文文案与亮点说明
 */
export const FEATURED_QUARK_UPDATES: ResourceItem[] = [
  // 0.《挑情丑闻 (2026)》
  {
    id: 'res-quark-movie-tiaoqing-chouwen-2026',
    title: '《挑情丑闻 (2026)》高清完整版 (最新热播悬疑/爱情剧情电影)',
    mainCategoryId: 'video',
    subCategoryId: 'movie',
    subCategoryName: '电影',
    subsiteId: 'dy',
    subsiteName: '影视短剧资源站',
    subsiteUrl: 'http://dy.ngy123.com',
    category: 'dy',
    categoryName: '剧情 / 爱情 / 悬疑惊悚',
    driveType: 'quark',
    driveName: '夸克网盘',
    driveUrl: 'https://pan.quark.cn/s/d920315b2450',
    extractCode: '/~78d63b2FdE~:/',
    quality: '1080P/4K 高清原画完整版 / 官方中字',
    size: '4.5 GB',
    sizeBytes: 4831838208,
    publishDate: '2026-09-22 09:00',
    relativeTime: '最新上架',
    views: 18600,
    downloads: 7200,
    isFeatured: true,
    isLatest: true,
    tags: ['挑情丑闻', '挑情丑闻2026', '2026电影', '剧情片', '爱情电影', '悬疑', '夸克网盘', '今日最新', '最新上架'],
    description: '【2026最新影视精选】《挑情丑闻》（2026）高清完整版，剧情曲折跌宕，交织复杂人性与欲望纠葛。夸克网盘极速免封转存，支持手机APP口令【/~78d63b2FdE~:/】与直达链接秒级转存收藏！',
    recommendation: `🎬 《挑情丑闻 (2026)》高清完整版

2026最新热门剧情反转电影《挑情丑闻》，聚焦欲望与纠葛，剧情跌宕起伏。

我用夸克网盘给你分享了「挑情丑闻（2026）」，点击链接或复制整段内容，打开「夸克APP」即可获取。
口令：/~78d63b2FdE~:/
链接：https://pan.quark.cn/s/d920315b2450

更多资源访问 www.wangpan8.com 网盘吧`
  },
  // 1.《求救信号 Mayday (2026)》
  {
    id: 'res-quark-movie-mayday-2026',
    title: '《求救信号 Mayday (2026)》4K 杜比视界·全景声顶级原盘压制版 (2160p.ATVP.WEB-DL.DDP5.1.Atmos.DV.H.265-SmartWeb)',
    mainCategoryId: 'video',
    subCategoryId: 'movie',
    subCategoryName: '电影',
    subsiteId: 'dy',
    subsiteName: '影视短剧资源站',
    subsiteUrl: 'http://dy.ngy123.com',
    category: 'dy',
    categoryName: '悬疑惊悚 / 灾难电影',
    driveType: 'quark',
    driveName: '夸克网盘',
    driveUrl: 'https://pan.quark.cn/s/fcec847516f4',
    extractCode: '/~d9623awnPg~:/',
    quality: '4K 2160p · Apple TV+ (ATVP) · 杜比视界 (DV) · 杜比全景声 (Atmos DDP5.1) · H.265 · SmartWeb',
    size: '16.8 GB',
    sizeBytes: 18038862643,
    publishDate: '2026-09-18 08:50',
    relativeTime: '刚刚更新',
    views: 46800,
    downloads: 18900,
    isFeatured: true,
    isLatest: true,
    tags: ['求救信号', 'Mayday', '2026电影', 'SmartWeb', '4K杜比视界', 'Dolby Atmos', 'ATVP', '悬疑惊悚', '灾难片', '夸克网盘', '极限反转', '今日最新'],
    description: '【2026年度惊悚反转大作·SmartWeb顶级母盘压制】《求救信号》(Mayday 2026) 2160p.ATVP.WEB-DL.DDP5.1.Atmos.DV.H.265-SmartWeb 顶级发烧原盘压制版！一场突如其来的空难危机，让原本平静的局面迅速失控。当求救信号出现却迟迟得不到回应，未知的危险与极端人性选择交织。Apple TV+ 官方片源，支持杜比视界（Dolby Vision）与杜比全景声（Dolby Atmos），视听压迫感拉满！口令【/~d9623awnPg~:/】直达！',
    recommendation: `🎬 《求救信号 Mayday (2026)》2160p.ATVP.WEB-DL.DDP5.1.Atmos.DV.H.265-SmartWeb

如果你喜欢悬疑、惊悚、灾难类电影，这部《求救信号 Mayday》绝对是年度必看之作。

一场突如其来的危机，让原本平静的局面迅速失控。当求救信号出现，却迟迟得不到回应，未知的危险、不断升级的紧张局势，以及人与人之间在极端环境下的选择，逐渐把故事推向高潮。

本版本为 SmartWeb 制作组打造的 Apple TV+ 4K 杜比视界（Dolby Vision）+ 杜比全景声（Dolby Atmos DDP5.1）顶级母盘压制版，大屏观影沉浸感十足。

我用夸克网盘给你分享了「求救信号Mayday.2026.2160p.ATVP.WEB-DL.DDP5.1.Atmos.DV.H.265-SmartWeb」。
口令：/~d9623awnPg~:/
链接：https://pan.quark.cn/s/fcec847516f4

更多资源访问 www.wangpan8.com 网盘吧`
  },

  // 2. 东野圭吾约105本作品合集
  {
    id: 'res-quark-books-higashino-keigo-105',
    title: '东野圭吾作品合集｜约105本打包 (精排EPUB/MOBI/PDF·含白夜行/嫌疑人X/解忧杂货店/恶意等全系列)',
    mainCategoryId: 'books',
    subCategoryId: 'ebooks',
    subCategoryName: '经典电子书 / 社科名著',
    subsiteId: 'tianya',
    subsiteName: '天涯神贴分享站',
    subsiteUrl: 'http://tianya.ngy123.com',
    category: 'tianya',
    categoryName: '推理悬疑小说 / 名著典藏',
    driveType: 'quark',
    driveName: '夸克网盘',
    driveUrl: 'https://pan.quark.cn/s/3e02bdac2bc5',
    extractCode: '',
    quality: '全集精排 105本典藏打包',
    size: '3.8GB',
    publishDate: '2026-09-04',
    relativeTime: '重磅推荐',
    views: 41200,
    downloads: 7950,
    isFeatured: true,
    isLatest: true,
    isCollection: true,
    tags: ['东野圭吾', '作品合集', '105本打包', '日本推理', '白夜行', '嫌疑人X的献身', '恶意', '解忧杂货店', '新参者', '夸克网盘'],
    description: '如果你喜欢推理小说，那么东野圭吾基本是绕不开的名字。从《白夜行》《嫌疑人X的献身》《恶意》，到《解忧杂货店》《新参者》等作品，东野圭吾最厉害的地方，不只是设置一个复杂的谜题，而是经常把推理、犯罪、人性和情感揉在一起。他的故事往往从一个看似普通的案件开始，随着阅读深入，你会发现真正值得思考的并不只是“凶手是谁”，而是：一个人为什么会走到这一步？这次整理的是东野圭吾约105本作品合集，对于喜欢日本推理、悬疑小说，或者想系统阅读东野圭吾作品的朋友来说，非常适合一次性收藏。⚠️ 注意：资源提示必须先转存到自己的网盘，链接约24小时失效，共105册。',
    recommendation: `📚 东野圭吾作品合集｜约105本打包

如果你喜欢推理小说，那么东野圭吾基本是绕不开的名字。

从《白夜行》《嫌疑人X的献身》《恶意》，到《解忧杂货店》《新参者》等作品，东野圭吾最厉害的地方，不只是设置一个复杂的谜题，而是经常把推理、犯罪、人性和情感揉在一起。

他的故事往往从一个看似普通的案件开始，随着阅读深入，你会发现真正值得思考的并不只是“凶手是谁”，而是：一个人为什么会走到这一步？

这次整理的是东野圭吾约 105本作品合集，对于喜欢日本推理、悬疑小说，或者想系统阅读东野圭吾作品的朋友来说，非常适合一次性收藏。

⚠️ 注意：资源提示必须先转存到自己的网盘，链接约24小时失效，共105册。

链接：https://pan.quark.cn/s/3e02bdac2bc5

更多资源访问 www.wangpan8.com 网盘吧`
  },

  // 3.《少儿英语启蒙合集》
  {
    id: 'res-quark-edu-kids-english-collection',
    title: '《少儿英语启蒙合集》(儿歌/动画/基础词汇/日常表达/家庭英语启蒙资料库)',
    mainCategoryId: 'education',
    subCategoryId: 'school',
    subCategoryName: '中小学辅导 / 少儿启蒙',
    subsiteId: 'xuexi',
    subsiteName: '中小学学习资料网',
    subsiteUrl: 'http://xuexi.ngy123.com',
    category: 'xuexi',
    categoryName: '少儿英语启蒙 / 基础培优',
    driveType: 'quark',
    driveName: '夸克网盘',
    driveUrl: 'https://pan.quark.cn/s/a5d5c298cb76',
    extractCode: '',
    quality: '动画+儿歌音频+高清绘本词汇',
    size: '15.2GB',
    publishDate: '2026-09-04',
    relativeTime: '热选推荐',
    views: 32800,
    downloads: 5890,
    isFeatured: true,
    isLatest: true,
    isCollection: true,
    tags: ['少儿英语启蒙合集', '少儿英语', '英语启蒙', '儿歌动画', '基础词汇', '日常表达', '语感培养', '夸克网盘'],
    description: '孩子学英语，最重要的往往不是一开始就背大量单词，而是先建立对英语的兴趣、语感和理解能力。这套少儿英语启蒙合集，适合家长用来给孩子做日常英语启蒙。通过儿歌、动画、基础词汇、日常表达等内容，让孩子在相对轻松的环境里接触英语，把学习融入日常生活。对于年龄较小、刚开始接触英语的孩子来说，“多听、多看、多接触”往往比单纯背单词更加重要。家长可以根据孩子的年龄和英语基础，选择合适的内容循序渐进地使用。如果你正在给孩子寻找一套比较系统的英语启蒙资料，这个合集可以先收藏起来，作为家庭英语启蒙的资料库。',
    recommendation: `🧒 《少儿英语启蒙合集》

孩子学英语，最重要的往往不是一开始就背大量单词，而是先建立对英语的兴趣、语感和理解能力。

这套少儿英语启蒙合集，适合家长用来给孩子做日常英语启蒙。通过儿歌、动画、基础词汇、日常表达等内容，让孩子在相对轻松的环境里接触英语，把学习融入日常生活。

对于年龄较小、刚开始接触英语的孩子来说，“多听、多看、多接触”往往比单纯背单词更加重要。家长可以根据孩子的年龄和英语基础，选择合适的内容循序渐进地使用。

如果你正在给孩子寻找一套比较系统的英语启蒙资料，这个合集可以先收藏起来，作为家庭英语启蒙的资料库。

我用夸克网盘给你分享了「少儿英语启蒙合集」。
链接：https://pan.quark.cn/s/a5d5c298cb76

更多资源访问 www.wangpan8.com 网盘吧`
  }
];
