...
const DRIVE_OPTIONS: { id: DriveType; label: string; iconColor: string }[] = [
  { id: 'all', label: '全部网盘', iconColor: 'text-emerald-500' },
  { id: 'quark', label: '夸克网盘', iconColor: 'text-amber-500' },
  { id: 'baidu', label: '百度网盘', iconColor: 'text-blue-500' },
...
export const TopSearchBar: React.FC<TopSearchBarProps> = ({
  ...
  return (
    <section 
      ...
      className="w-full bg-gradient-to-b from-white via-white to-neutral-50/50 dark:from-neutral-900 dark:via-neutral-900 dark:to-neutral-900/60 rounded-3xl border border-neutral-200/90 dark:border-neutral-800 shadow-md p-4 sm:p-6 lg:p-7 space-y-4 transition-all"
    >
      ...
      <div className="relative max-w-4xl mx-auto">
        <div 
          className={`flex items-center w-full rounded-2xl bg-neutral-50 dark:bg-neutral-800/90 border-2 transition-all shadow-sm ${
            isFocused 
              ? 'border-emerald-500 dark:border-emerald-400 ring-4 ring-emerald-500/15 dark:ring-emerald-400/20 bg-white dark:bg-neutral-800' 
        ...
        >
          ...
          <div className="pl-4 sm:pl-5 pr-2 flex items-center justify-center text-neutral-400 dark:text-neutral-500 pointer-events-none">
            <Search className={`w-5 h-5 sm:w-6 sm:h-6 transition-colors ${isFocused ? 'text-emerald-500 dark:text-emerald-400' : ''}`} />
          </div>
          ...
          {searchQuery && (
            <button
              ...
              type="button"
              onClick={handleClear}
              className="p-1.5 sm:p-2 mr-1 rounded-full text-neutral-400 hover:text-neutral-700 dark:hover:text-neutral-200 hover:bg-neutral-200/60 dark:hover:bg-neutral-700 transition-colors"
              title="清空搜索内容"
            >
          ...
          <div className="pr-1.5 sm:pr-2">
            <button
              ...
              className="flex items-center gap-1.5 px-4 sm:px-6 py-2.5 sm:py-3 rounded-xl bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white font-bold text-sm sm:text-base shadow-sm hover:shadow transition-all cursor-pointer"
            >
      ...
      <div className="max-w-4xl mx-auto flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 pt-1">
        ...
        <div className="text-xs text-neutral-500 dark:text-neutral-400 font-mono flex items-center gap-1.5 shrink-0 self-end sm:self-auto">
          {isSearching ? (
            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-emerald-50 dark:bg-emerald-950/60 text-emerald-600 dark:text-emerald-400 font-bold border border-emerald-200 dark:border-emerald-800">
              <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" />
              匹配到 <strong className="text-emerald-700 dark:text-emerald-300">{filteredCount}</strong> 条资源
            </span>
          ) : (
